...
void LCD_PIN_Init(void){
  ...
  RCC->AHB2ENR |= RCC_AHB2ENR_GPIODEN;   
	...
	//Configuring Port A
	GPIOA->MODER &= ~(0xC03FFCFF); //Clear pins 0,1,2,3,5,6,7,8,9,10, and 15
	GPIOA->MODER |= (0x802AA00C);		//set above pins to Alternative function(10)
	
	GPIOA->PUPDR &=~(0xCFF);	//no pull up/pull down mask
	GPIOA->PUPDR |=(0x8AA);		//no pull up/pull down value
	
	GPIOA->AFR[0] &= ~(0xFF000000);	//Clear pins 6 and 7
	GPIOA->AFR[0] |= (0xBB000000);	//Set above pins to 0xB

	GPIOA->AFR[1] &= ~(0xF0000FFF);	//Clear pins 8,9,10, and 15
	GPIOA->AFR[1] |= (0xB0000BBB);	//Set above pins to 0xB

//Configuring Port B
	GPIOB->MODER &= ~(0xFF0C0F0F); //Clear pins 0,1,4,5,9,12,13,14,and 15
	GPIOB->MODER |= (0xAA080A0A);		//set above pins to Alternative function(10)
	
	GPIOB->AFR[0] &= ~(0x00FF00FF);	//Clear pins 0,1,4,and 5
	GPIOB->AFR[0] |= (0x00BB00BB);	//Set above pins to 0xB

	GPIOB->AFR[1] &= ~(0xFFFF00F0);	//Clear pins 9,12,13,14, and 15
	GPIOB->AFR[1] |= (0xBBBB00B0);	//Set above pins to 0xB
	
	//Configuring Port C
	GPIOC->MODER &= ~(0x0003FFC0); //Clear pins 3,4,5,6,7, and 8
	GPIOC->MODER |= (0x0002AA80);		//set above pins to Alternative function(10)
	
	GPIOC->AFR[0] &= ~(0xFFFFF000);	//Clear pins 3,4,5,6,and 7
	GPIOC->AFR[0] |= (0xBBBBB000);	//Set above pins to 0xB

	GPIOC->AFR[1] &= ~(0xF);	//Clear pin 8
	GPIOC->AFR[1] |= (0xB);	//Set above pin to 0xB
	
	//Configuring Port D
	GPIOD->MODER &= ~(0xFFFF0000); //Clear pins 8,9,10,11,12,13,14,and 15
	GPIOD->MODER |= (0xAAAA0000);		//set above pins to Alternative function(10)
	
	GPIOD->AFR[0] &= ~(0x0);	//Clear nothing
	GPIOD->AFR[0] |= (0x0);	

	GPIOD->AFR[1] &= ~(0xFFFFFFFF);	//Clear pins  8,9,10,11,12,13,14,and 15
	GPIOD->AFR[1] |= (0xBBBBBBBB);	//Set above pins to 0xB

}

void LCD_DisplayStringlong(uint8_t* ptr){
	int i,j,k;
	while (1)
	{
		for (j=0;j<18;j++)
		{
			...
			for (i=0;i<6;i++) 
			{	
				LCD_WriteChar(&ptr[i+j],0,0,i);
			}
		}	
	}
}
void LCD_DisplayString(uint8_t* ptr, int len){
	int i,j,size,begin;
	if (len>5)
	{
		size=len;
...
void LCD_Display_Name(void){
	...
	LCD->RAM[0] &= ~(1U<<4|1U<<6|1U<<9|1U<<12|1U<<14|1U<<15|1U<<17|1U<<30|1U<<31);
	LCD->RAM[2] &= ~(1U<<3|1U<<4|1U<<5|1U<<6|1U<<8|1U<<13|1U<<14|1U<<24|1U<<26|1U<<28|1U<<30);
	LCD->RAM[3] &= ~(1U<<3);
	LCD->RAM[5] &= ~(1U|1U<<3);
	LCD->RAM[6] &= ~(1U<<17|1U<<24|1U<<26|1U<<31);
	
	LCD->RAM[0] |= (0xC002D250);
	LCD->RAM[2] |= (0x55006178);
	LCD->RAM[3] |= (0x8);
	LCD->RAM[5] |= (0x9);
	LCD->RAM[6] |= (0x85020000);
...
void LCD_Configure(void){
	LCD->CR &= ~(LCD_CR_BIAS); //Clear the Bias 
	LCD->CR |= (LCD_CR_BIAS_1);	//Set bias to 1/3
	
	LCD->CR &= ~(LCD_CR_DUTY);	//Clear duty
	LCD->CR |= (LCD_CR_DUTY_0);
	LCD->CR |= (LCD_CR_DUTY_1);
	LCD->CR |= (LCD_CR_DUTY_2);		//Set duty to 1/4
	
	LCD->FCR &= ~(LCD_FCR_CC);	//Clear Contrast control
	LCD->FCR |= (LCD_FCR_CC);	//Set Contrast control to max
	
	LCD->FCR &= ~(LCD_FCR_PON);	//Clear Pulse on Period
	LCD->FCR |= (LCD_FCR_PON);	//Set Pulse on Period to 111
	
	LCD->CR &= ~(LCD_CR_MUX_SEG); //Disable Mux Seg 
	
	LCD->CR &= ~(LCD_CR_VSEL);	//Set voltage source to internal
	
	while (!(LCD->SR & LCD_SR_FCRSR)){}	//wait for FCRSR flag is set
	
	LCD->CR &= ~(LCD_CR_LCDEN);		//Clear LCDEN
	LCD->CR |= LCD_CR_LCDEN;		//Set LCDEN
	
	while (!(LCD->SR & LCD_SR_ENS)) {}	//Wait until ENS is enabled
	
	while (!(LCD->SR & LCD_SR_RDY)) {}	//Wait until Ready bit is enabled
